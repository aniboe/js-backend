...
const crypto = require("crypto") // dont know what it does
...
app.post("/upload", upload.single("image"), (req, res) => {
    // console.log(req.body)  // for text
    console.log(req.file)
    if(req.file) return res.send("file uploaded susseskdfhaoiwesf")
...
app.get("/like/:id", isLoggedIn, async (req, res) => {
    ...
    if(post.likes.indexOf(req.user.userid) === -1){
        post.likes.push(req.user.userid) // it takes value from midilware(that has jwt)
    }
    else{
